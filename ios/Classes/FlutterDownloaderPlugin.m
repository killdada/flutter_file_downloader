...
@interface FlutterDownloaderPlugin()<NSURLSessionTaskDelegate, NSURLSessionDownloadDelegate, UIDocumentInteractionControllerDelegate>
{
    ...
    NSMutableDictionary *_speedTimeDic;
}
...
- (instancetype)initWithBinaryMessenger: (NSObject<FlutterBinaryMessenger>*) messenger;
{
    if (self = [super init]) {
        ...
        _speedTimeDic = [NSMutableDictionary dictionary];
        // init NSURLSession
...
- (void)sendUpdateProgressForTaskId: (NSString*)taskId inStatus: (NSNumber*) status andProgress: (NSNumber*) progress andSpeed:(NSNumber *)speed
{
...
openFileFromNotification: (BOOL) openFileFromNotification
{
    ...
    extra = [self escape:extra revert:false];
    NSString *query = [NSString stringWithFormat:@"INSERT INTO task (task_id,url,status,progress,file_name,saved_dir,headers,extra,resumable,show_notification,open_file_from_notification,time_created) VALUES (\"%@\",\"%@\",%d,%d,\"%@\",\"%@\",\"%@\",\"%@\",%d,%d,%d,%ld)", taskId, url, status, progress, filename, savedDir, headers,extra, resumable ? 1 : 0, showNotification ? 1 : 0, openFileFromNotification ? 1 : 0, [self currentTimeInMilliseconds]];
...
- (NSDictionary*) taskDictFromRecordArray:(NSArray*)record
{
    ...
    NSString *extra = [record objectAtIndex:[_dbManager.arrColumnNames indexOfObject:@"extra"]];
    extra = [self escape:extra revert:true];
    ...
    return [NSDictionary dictionaryWithObjectsAndKeys:taskId, KEY_TASK_ID, @(status), KEY_STATUS, @(progress), KEY_PROGRESS, url, KEY_URL, filename, KEY_FILE_NAME, headers, KEY_HEADERS, savedDir, KEY_SAVED_DIR, [NSNumber numberWithBool:(resumable == 1)], KEY_RESUMABLE, [NSNumber numberWithBool:(showNotification == 1)], KEY_SHOW_NOTIFICATION, [NSNumber numberWithBool:(openFileFromNotification == 1)], KEY_OPEN_FILE_FROM_NOTIFICATION, @(timeCreated), KEY_TIME_CREATED,extra,KEY_EXTRA,nil];
}
...
- (void)enqueueMethodCall:(FlutterMethodCall*)call result:(FlutterResult)result {
    ...
    NSString *extra = call.arguments[KEY_EXTRA];
    ...
    __typeof__(self) __weak weakSelf = self;
    dispatch_sync(databaseQueue, ^{
        [weakSelf addNewTask:taskId url:urlString status:STATUS_ENQUEUED progress:0 filename:fileName savedDir:shortSavedDir headers:headers extra:extra resumable:NO showNotification: [showNotification boolValue] openFileFromNotification: [openFileFromNotification boolValue]];
    });
    result(taskId);
 ...
 totalBytesWritten:(int64_t)totalBytesWritten totalBytesExpectedToWrite:(int64_t)totalBytesExpectedToWrite
{
    if (totalBytesExpectedToWrite == NSURLSessionTransferSizeUnknown) {
        ...
        NSDate *beforeDate = _speedTimeDic[taskId][@"date"];
        if (beforeDate == nil) {

            [_speedTimeDic setObject:@{@"date":[NSDate date],@"totalBytesWritten":@(totalBytesWritten)
                                       } forKey:taskId];
        }else if (-[beforeDate timeIntervalSinceDate:[NSDate date]] >= 1) {

            NSNumber *written = _speedTimeDic[taskId][@"totalBytesWritten"];
            float beforeWritten = [written floatValue];
            float speed = totalBytesWritten-beforeWritten;
            [self sendUpdateProgressForTaskId:taskId inStatus:@(STATUS_RUNNING) andProgress:@(progress) andSpeed:@(speed)];
            _runningTaskById[taskId][KEY_PROGRESS] = @(progress);

            NSDictionary *dic = @{@"date":[NSDate date],@"totalBytesWritten":@(totalBytesWritten)
                                  };
            [_speedTimeDic setObject:dic forKey:taskId];
        }else{
