...
/// range of 0 and 100
///
typedef void DownloadCallback(String id, DownloadTaskStatus status, int progress,double speed);
///
...
class FlutterDownloader {
  ...
  static Future<String> enqueue(
    ...
    StringBuffer extraBuilder = StringBuffer();
    if (extraBuilder != null) {
      extraBuilder.write('{');
      extraBuilder.writeAll(
          extra.entries
              .map((entry) => '\"${entry.key}\": \"${entry.value}\"'),
          ',');
      extraBuilder.write('}');
    ...
    if (callback != null) {
      ...
      platform.setMethodCallHandler((MethodCall call) {
        if (call.method == 'updateProgress') {
          ...
          double speed = call.arguments['speed'];
          callback(id, DownloadTaskStatus._internal(status), process,speed);
        }
        return null;
      });
    } else {
